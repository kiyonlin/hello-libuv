...
void run_tcp_echo_server() {
    ...
    uv_ip4_addr("0.0.0.0", DEFAULT_PORT, &addr_tcp);

    uv_tcp_bind(&server, (const struct sockaddr *) &addr_tcp, 0);

    // 第二个参数是待处理的连接请求队列－最大长度的请求连接队列。
    int r = uv_listen((uv_stream_t *) &server, DEFAULT_BACKLOG, on_new_tcp_connection);
